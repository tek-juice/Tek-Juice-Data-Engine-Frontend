    ...
    <div style={{ border: '1px solid var(--border)', borderRadius: 6, background: 'var(--surface)', overflow: 'hidden' }}>
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-3)' }}>{draft.intent} · {draft.word_count} words</p>
        </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3" style={{ borderTop: '1px solid var(--border)' }}>
          ...
          {draft.projected_position && (
            <p className="text-xs" style={{ color: 'var(--text-3)' }}>Projected position: <span style={{ color: 'var(--text)', fontWeight: 600 }}>{draft.projected_position}</span></p>
          )}
        </div>
      )}
    </div>
      ...
      {open && (
        <div className="px-4 pb-4" style={{ borderTop: '1px solid var(--border)' }}>
          ...
          {!loading && !err && drafts.length === 0 && (
            <p className="text-xs pt-3" style={{ color: 'var(--text-3)' }}>No drafts generated yet for this document.</p>
          )}
          {drafts.length > 0 && (
    ...
    <div className="px-7 py-7 max-w-3xl" style={{ background: 'var(--bg)', color: 'var(--text)', minHeight: '100vh' }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: 'var(--text-3)' }}>AI-generated content drafts per document — expand to read and review.</p>
        </div>
