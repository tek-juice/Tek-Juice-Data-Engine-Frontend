...
export default function TenantDashboard() {
  ...
  useEffect(() => {
    ...
    getQualityScore({
      ...
      title: t.name + ' — Data Automation Platform',
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
          ...
          {liveQS !== null && (
            <StatCard
              ...
              icon={<Target size={14}/>}
              accent={qsData?.beats_paid_ads ? 'text-emerald-400' : 'text-amber-300'}
            />
          )}
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-zinc-600">— QS ≥ 8 ranks above paid ads · QS 9.5+ enters AI Overviews</span>
            </div>
          ...
          {qsData ? (
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                {/* Big QS number */}
                <div className={`p-3 border rounded-sm col-span-2 md:col-span-1 ${
                  ...
                  <div className="text-xs text-zinc-500 uppercase font-medium tracking-wide mb-1">Quality Score</div>
                  ...
                  }`}>{liveQS?.toFixed(1)}<span className="text-base text-zinc-600 ml-1">/10</span></div>
                  <div className={`text-xs font-semibold mt-2 ${qsData.beats_paid_ads ? 'text-emerald-400' : 'text-zinc-400'}`}>
                    {qsData.label}{qsData.beats_paid_ads ? ' · Beats paid ads' : ''}
                  </div>
                  <div className="text-xs text-zinc-600 mt-0.5 leading-tight">{qsData.projected_position}</div>
                  <div className="mt-3 grid grid-cols-2 gap-1.5 text-xs font-mono">
                    <div>
                      <span className="text-zinc-600">CTR</span>
                      <span className="text-zinc-300 ml-1">{(qsData.rank_simulation.estimated_ctr * 100).toFixed(1)}%</span>
                    </div>
                    <div>
                      <span className="text-zinc-600">Ads beaten</span>
                      <span className="text-zinc-300 ml-1">{qsData.rank_simulation.paid_ads_beaten}/4</span>
                    </div>
                  </div>
                </div>
                {/* 3 dimension bars */}
                {Object.entries(qsData.dimensions).map(([key, dim]) => (
                  <div key={key} className="border border-zinc-800 p-3 flex flex-col gap-2">
                    ...
                    <div className="flex items-end gap-1">
                      ...
                      <span className="text-zinc-600 text-xs mb-0.5">/10</span>
                    </div>
                    ...
                    {dim.fixes[0] && (
                      <div className="text-xs text-zinc-600 leading-tight mt-1 border-t border-zinc-800 pt-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-3 border-t border-zinc-800">
                <div>
                  <div className="text-xs font-mono text-zinc-500 mb-2">vs Paid Ad Positions</div>
                  <div className="space-y-1.5">
                    {qsData.rank_simulation.ad_benchmarks.map(b => (
                      <div key={b.ad_position} className="flex items-center gap-2 text-xs font-mono">
                        ...
                        <span className="text-zinc-700">QS {b.ad_typical_qs}</span>
                        {b.beats_this_ad
                          ? <span className="text-emerald-400 ml-auto">BEATS ✓</span>
                          : <span className="text-red-400 ml-auto">+{b.qs_gap} pts</span>
                        }
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <div className="text-xs font-mono text-zinc-500 mb-2">Uplift Path to #1</div>
                  <div className="space-y-1.5">
                    {qsData.rank_simulation.uplift_steps.slice(0, 4).map((s, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs font-mono">
                        <span className="text-zinc-700">{s.from_qs}→{s.to_qs}</span>
                        <span className="text-zinc-400 flex-1">{s.milestone}</span>
                        <span className="text-emerald-600">{s.ctr_gain}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="grid grid-cols-4 gap-3">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="h-32 bg-zinc-800/50 animate-pulse rounded-sm" />
              ))}
            </div>
          )}
        </div>
